"""Command-line interface helpers surfaced via the ``gabriel`` executable."""
...
from gabriel import arithmetic as _arithmetic
from gabriel.common import secrets as secrets_module
...
def main(argv: list[str] | None = None) -> None:
    ...
    funcs = {
        "add": _arithmetic.add,
        "subtract": _arithmetic.subtract,
        "multiply": _arithmetic.multiply,
        "divide": _arithmetic.divide,
        "power": _arithmetic.power,
        "modulo": _arithmetic.modulo,
        "floordiv": _arithmetic.floordiv,
    }

    if args.command == "sqrt":
        print(_arithmetic.sqrt(args.a))
        return
    ...
    if args.command == "viewer":
        # Defer the shim import so monkeypatched ``gabriel.utils`` hooks are respected.
        utils_module = importlib.import_module("gabriel.utils")
        utils_module.serve_viewer(
            host=args.host,
            port=args.port,
